...
# back flow map
T_x = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y, res_z))  # d_psi / d_x
T_y = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x, res_y + 1, res_z))  # d_psi / d_y
T_z = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x, res_y, res_z + 1))  # d_psi / d_z

T_x_e = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x, res_y + 1, res_z + 1))
T_y_e = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y, res_z + 1))
T_z_e = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y + 1, res_z))
T_n = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y + 1, res_z + 1))  # d_psi / d_z

psi_x = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y, res_z))
psi_y = ti.Vector.field(3, ti.f32, shape=(res_x, res_y + 1, res_z))
psi_z = ti.Vector.field(3, ti.f32, shape=(res_x, res_y, res_z + 1))

psi_x_e = ti.Vector.field(3, ti.f32, shape=(res_x, res_y + 1, res_z + 1))
psi_y_e = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y, res_z + 1))
psi_z_e = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y + 1, res_z))

# fwrd flow map
F_x = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y, res_z))  # d_phi / d_x
F_y = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x, res_y + 1, res_z))  # d_phi / d_y
F_z = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x, res_y, res_z + 1))  # d_phi / d_z

F_x_e = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x, res_y + 1, res_z + 1))
F_y_e = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y, res_z + 1))
F_z_e = ti.Matrix.field(n=3, m=3, dtype=ti.f32, shape=(res_x + 1, res_y + 1, res_z))

phi_x = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y, res_z))
phi_y = ti.Vector.field(3, ti.f32, shape=(res_x, res_y + 1, res_z))
phi_z = ti.Vector.field(3, ti.f32, shape=(res_x, res_y, res_z + 1))

phi_x_e = ti.Vector.field(3, ti.f32, shape=(res_x, res_y + 1, res_z + 1))
phi_y_e = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y, res_z + 1))
phi_z_e = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y + 1, res_z))
...
# vorticity storage (on face fields and node fields)
...
w_n = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y + 1, res_z + 1))
w_n_tmp = ti.Vector.field(3, ti.f32, shape=(res_x + 1, res_y + 1, res_z + 1))
# some helper storage for w, on edge fields
# on edge fields
# on face fields
...
def backtrack_psi_grid(curr_step):
    ...
    for step in reversed(range(curr_step)):
        RK4_neural(psi_x, F_x, step)
        RK4_neural(psi_y, F_y, step)
        RK4_neural(psi_z, F_z, step)
...
# u_x1, u_y1, u_z1 are the current time quantities (to be modified)
@ti.kernel
def advect_u(
    u_x0: ti.template(),
    u_y0: ti.template(),
...
def main():
    ...
    while True:
        ...
        interp_f2e(F_x, F_y, F_z, F_x_e, F_y_e, F_z_e)
        interp_f2e(psi_x, psi_y, psi_z, psi_x_e, psi_y_e, psi_z_e)
        ...
        interp_f2e(F_x, F_y, F_z, F_x_e, F_y_e, F_z_e)
        interp_f2e(T_x, T_y, T_z, T_x_e, T_y_e, T_z_e)
        interp_f2e(psi_x, psi_y, psi_z, psi_x_e, psi_y_e, psi_z_e)
        interp_f2e(phi_x, phi_y, phi_z, phi_x_e, phi_y_e, phi_z_e)
        ...
        advect_w_notrans(
            ...
            F_x_e, F_y_e, F_z_e, psi_x_e, psi_y_e, psi_z_e,
            dx,
        )
        # # Begin BFECC
        advect_w_notrans(
            w_x, w_y, w_z, err_w_x, err_w_y, err_w_z,
            T_x_e, T_y_e, T_z_e, phi_x_e, phi_y_e, phi_z_e,
            dx,
        )
        ...
        BFECC_limiter(err_w_x, w_x, tmp_w_x)
        BFECC_limiter(err_w_y, w_y, tmp_w_y)
        BFECC_limiter(err_w_z, w_z, tmp_w_z)
        copy_to(tmp_w_x, w_x)
        copy_to(tmp_w_y, w_y)
        copy_to(tmp_w_z, w_z)
